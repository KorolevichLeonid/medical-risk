...
const DocumentView = () => {
  ...
  const handleGenerateDocument = async () => {
    ...
    if (!confirm('Создать новую версию отчета по управлению рисками?')) return;
  ...
  const handleDownloadDocument = async (versionId, format = 'docx') => {
    try {
      ...
      console.log('Статус ответа скачивания:', response.status);

      if (!response.ok) {
        const errorText = await response.text();
        console.error('Скачивание не удалось:', errorText);
      ...
      console.log(`Скачивание ${format.toUpperCase()} файла как:`, filename);
  ...
  return (
    <div className="document-view">
        ...
        {selectedVersion ? (
          <div className="document-preview">
            <div className="document-info-card">
              <div className="document-badge-container">
                ...
                {selectedVersion.is_current && (
                  <span className="badge badge-current">Текущая версия</span>
                )}
              </div>
            ...
            <div className="no-document">
              ...
              <h2>Документ не сгенерирован</h2>
              <p>Для этого проекта еще не был сгенерирован отчет по управлению рисками.</p>
              <button 
                className="btn btn-primary btn-large"
                onClick={handleGenerateDocument}
                disabled={generating}
              >
                {generating ? 'Генерация...' : 'Создать первую версию'}
              </button>
            </div>
        )}

        {/* Сайдбар истории версий */}
              ...
              {versions.map((version) => (
                ...
                >
                  <div className="version-header">
                    ...
                    {version.is_current && (
                      <span className="badge badge-current-small">Текущая</span>
                    )}
                  </div>
      ...
      {/* Плавающие кнопки скачивания - показываются только если документ существует */}
      {selectedVersion && (
