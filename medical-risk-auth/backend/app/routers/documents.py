...
def generate_html_preview(project, doc_version, risks, team_members):
    ...
    # Счётчики до мер (первичная оценка)
    evaluated_initial = 0
    acceptable_before = 0       # приемлимы сразу до мер
    unacceptable_before = 0     # неприемлимы до мер (нужны меры)

    # Счётчики после мер (остаточная оценка) — только для тех, у кого есть остаточные баллы
    evaluated_residual = 0
    acceptable_after = 0        # стали приемлимыми после мер
    unacceptable_after = 0      # всё ещё неприемлимы после мер
    not_evaluated_after = 0     # неприемлимы до мер, но остаточная оценка отсутствует

    # Риски, закрытые через анализ польза/риск (несмотря на неприемлимый балл)
    closed_via_benefit = 0
                ...
                if initial_score < risk_threshold:
                    ...
                    if s_res is not None and p_res is not None and s_res > 0 and p_res > 0:
                        ...
                        if residual_score < risk_threshold:
                            ...
                            # Проверяем, закрыт ли через анализ польза/риск
